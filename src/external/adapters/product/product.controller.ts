...
import {
  ...
  CreatedProductSwagger,
  EditProductSwagger,
  LoadedCategoriesSwagger,
  LoadedProductsByCategorySwagger,
} from "src/internal/application/docs/swagger/product/create-product.dto";
import { DecreaseProductQuantity } from "src/internal/application/useCases/product/decrease-product-quantity.usecase";
...
export class ProductController {
  constructor(
    ...
    private readonly decreaseProductQuantity: DecreaseProductQuantity,
  ) {}
  ...
  async checkProductQuantity(
    @Body() checkProductQuantityDto: VerifyProductDto[],
  ) {
    try {
      return await this.decreaseProductQuantity.execute(
        checkProductQuantityDto,
      );
    } catch (err: any) {
      return responseError(err);
    }
  ...
  @ApiResponse({
    status: 200,
    description: "products by categories successfully loaded.",
    type: LoadedProductsByCategorySwagger,
  })
  @Get("category/:id")
  ...
  @ApiResponse({
    status: 200,
    description: "Categories successfully loaded.",
    type: LoadedCategoriesSwagger,
  })
  @Get("category/")
  async getCategories() {
    try {
